...
use App\Libraries\Repositories\GeneratorRepository;
use Flash;
...
class GeneratorController extends AppBaseController
{
	...
	function __construct(GeneratorRepository $generatorRepo)
	{
		$this->generatorRepository = $generatorRepo;
	}
	...
	{
		$generators = $this->generatorRepository->paginate(10);
	...
	{
		$generator = $this->generatorRepository->find($id);

		if(empty($generator))
		{
			Flash::error('Generator not found');

			return redirect(route('admin.generators.index'));
		}

		return view('generators.show')->with('generator', $generator);
	}
	...
	public function edit($id)
	{
		...
		return view('generators.edit')->with('generator', $generator);
	}
	...
	public function update($id, UpdateGeneratorRequest $request)
	{
		...
		$this->generatorRepository->updateRich($request->all(), $id);

		Flash::success('Generator updated successfully.');
	...
	public function destroy($id)
	{
		...
		Flash::success('Generator deleted successfully.');
